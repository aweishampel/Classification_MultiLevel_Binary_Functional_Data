...
core_function = function(i){
  ...
  Curves_train = rbind(Curves_gtrain, Curves_FStrain)
  Curves_test = rbind(Curves_gtest, Curves_FStest)

  #D = dim(Curves_train)[2]
  
  #Two Levels Levels Can easily change to 4 levels by changing the levels
  Classes_train = c(rep(1, N_gtrain), rep(2, N_rtrain))
  Classes_train = as.factor(Classes_train)
  Classes_test = c(rep(1, N_genuine-N_gtrain), rep(2, N_r-N_rtrain))
